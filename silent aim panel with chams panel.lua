...
if getgenv().ChamsScriptRunning then return end
getgenv().ChamsScriptRunning = true
...
-- Create a unique namespace for UI to avoid conflicts
local guiName = "ChamsScriptGUI"
local existingGui = player:FindFirstChild("PlayerGui") and player.PlayerGui:FindFirstChild(guiName)

if existingGui then
    existingGui:Destroy()  -- Destroy old instances to avoid duplication
end
...
pickButton.Size = UDim2.new(0, 100, 0, 50)
pickButton.Position = UDim2.new(0.5, -50, 0.1, 0)
pickButton.Text = "Pick a Color"
pickButton.Parent = frame

local rainbowButton = Instance.new("TextButton")
rainbowButton.Size = UDim2.new(0, 100, 0, 50)
rainbowButton.Position = UDim2.new(0.5, -50, 0.3, 0)
rainbowButton.Text = "Rainbow Mode"
rainbowButton.Parent = frame

local colorDisplay = Instance.new("Frame")
colorDisplay.Size = UDim2.new(0, 100, 0, 50)
colorDisplay.Position = UDim2.new(0.5, -50, 0.5, -25)
colorDisplay.BackgroundColor3 = Color3.new(1, 1, 1)
colorDisplay.Parent = frame

local colorLabel = Instance.new("TextLabel")
colorLabel.Size = UDim2.new(0, 200, 0, 50)
colorLabel.Position = UDim2.new(0.5, -100, 0.8, 0)
colorLabel.Text = ""
colorLabel.Parent = frame

-- FOV Circle (Safe Persistent Instance)
if not getgenv().ChamsFOVCircle then
    getgenv().ChamsFOVCircle = Drawing.new("Circle")
end

local fov_circle = getgenv().ChamsFOVCircle
fov_circle.Thickness = 1
fov_circle.NumSides = 100
fov_circle.Radius = 180
fov_circle.Filled = false
...
local function createESP(plr)
    if plr ~= player then
        plr.CharacterAdded:Connect(function(character)
            ...
            highlight.FillColor = fov_circle.Color
            highlight.FillTransparency = 0.5
        end)

        -- If the player is already loaded, apply ESP immediately
...
local function pick_color()
    ...
    colorLabel.Text = string.format("RGB: %d, %d, %d", randomColor.R * 255, randomColor.G * 255, randomColor.B * 255)
    fov_circle.Color = randomColor
    for _, plr in pairs(Players:GetPlayers()) do
        local highlight = plr.Character and plr.Character:FindFirstChildOfClass("Highlight")
        if highlight then
...
pickButton.MouseButton1Click:Connect(pick_color)
...
rainbowButton.MouseButton1Click:Connect(function()
    ...
    rainbowButton.Text = rainbowMode and "Rainbow: ON" or "Rainbow: OFF"
end)

-- Toggle UI Visibility with Left Ctrl
local uiVisible = true
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if input.KeyCode == Enum.KeyCode.LeftControl and not gameProcessed then
        uiVisible = not uiVisible
        screenGui.Enabled = uiVisible
        fov_circle.Visible = uiVisible
    end
end)

-- Update FOV Circle and Apply Rainbow Effect
RunService.RenderStepped:Connect(function()
    ...
    if rainbowMode then
        local newColor = updateRainbow()
        colorDisplay.BackgroundColor3 = newColor
        colorLabel.Text = string.format("RGB: %d, %d, %d", newColor.R * 255, newColor.G * 255, newColor.B * 255)
        fov_circle.Color = newColor
        for _, plr in pairs(Players:GetPlayers()) do
            local highlight = plr.Character and plr.Character:FindFirstChildOfClass("Highlight")
            if highlight then
